...
{
    try {
        if (!empty(($_POST['nom'] AND $_POST['prenom'] AND $_POST['mail']))) {
            function existeIdentifiant($identifiant)
            {
                $bdd = my_pdo_connexxionWEcon();
                $reqpseudo = $bdd->prepare("SELECT * FROM sousmembres WHERE nom = ?");
            ...
            if (existeIdentifiant($_POST['nom'])) {
                ...
                $req->execute(array(0, $_POST['nom'], $_POST['prenom'], $_POST['mail'], $_SESSION['id']));
                return (TRUE);
            }
        } else {
